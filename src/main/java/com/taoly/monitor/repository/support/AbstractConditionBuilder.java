...
public abstract class AbstractConditionBuilder<T> {
    ...
    protected void addIsNullConditionToColumn(QueryWraper<T> queryWraper, String column) {
        Path<?> fieldPath = getPath(queryWraper.getRoot(), column);
        queryWraper.addPredicate(queryWraper.getCb().isNull(fieldPath));
    }

//    protected void addOrLikeConditionToColumn(QueryWraper<T> queryWraper, String column, String value) {
//        queryWraper.addPredicate(queryWraper.getCb().or(createLikeCondition(queryWraper, column, value)));
//    }
